...
class Search():
  def __init__(self):
    ...
    def syncAds(self):
      """Get all ads for all searchItems from Ebay.

      Compares all ads with past marked and deleted ads. Store them in
      self.listTempAds. """

      for i in self.listSearchItems:
        #self.listTempDicts =     # no value given, produces error!!!
        self.parser.getAllAds(self.translator.searchItemToSearchItemDict(i))
        for j in self.listTempDicts:
          ...
          for i in self.listTempAds:
            if ((i not in self.listMarkedAds) and (i not in self.listDeletedAds)
              and (i not in self.listPassedAds)):
              i.setAdStatus('new')
    ...
    def addSearchItem(self, _searchItemDict):
      ...
      self.tempSearchItem = self.translator.searchItemDictToSearchItem(
          _searchItemDict)
